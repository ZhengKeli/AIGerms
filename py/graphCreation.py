...
weights_actor1 = tf.Variable(tf.random_uniform([7, 30], -1.0, 1.0), name="weights_actor1")
...
weights_critic1 = tf.Variable(tf.random_uniform([9, 50], -1.0, 1.0), name="weights_critic1")
bias_critic1 = tf.Variable(tf.random_uniform([50], -1.0, 1.0), name="bias_critic1")
output_critic1 = tf.nn.tanh(tf.matmul(input_critic1, weights_critic1) + bias_critic1, "output_critic1")  # [-1,50]

input_critic2 = output_critic1
weights_critic2 = tf.Variable(tf.random_uniform([50, 50], -1.0, 1.0), name="weights_critic2")
bias_critic2 = tf.Variable(tf.random_uniform([50], -1.0, 1.0), name="bias_critic2")
output_critic2 = tf.nn.tanh(tf.matmul(input_critic2, weights_critic2) + bias_critic2, "output_critic2")  # [-1,50]

input_critic3 = output_critic2
weights_critic3 = tf.Variable(tf.random_uniform([50, 1], -1.0, 1.0), name="weights_critic3")
...
# train
ass_loss = tf.multiply(tf.reduce_sum(output_critic3, -1), 1.0, name="ass_loss")  # [-1]
