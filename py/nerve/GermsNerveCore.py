...
class GermsNerveCore(NerveCore):
    ...
    def create_graph(self):
        with self.graph.as_default():
            feel_network = FullConnectedNetwork([3, 16, 16, 15], tf.nn.tanh)
            act_network = FullConnectedNetwork([15, 16, 16, 8], tf.nn.tanh)
            critic_network = FullConnectedNetwork([16, 16, 8], tf.nn.tanh)
            ...
            act_x = act_network.apply(perception_x)  # [-1,8]
            act_x = tf.reduce_mean(act_x, -1, keep_dims=True)  # [-1,1]
            act_y = act_network.apply(perception_y)  # [-1,8]
            act_y = tf.reduce_mean(act_y, -1, keep_dims=True)  # [-1,1]

            tf.concat([act_x, act_y], -1, "act")  # [-1,2]

            # critic network
            situation_x = tf.concat([perception_x, act_x], 1)  # [-1,16]
            situation_y = tf.concat([perception_y, act_y], 1)  # [-1,16]
            ...
            ass_loss = tf.reduce_mean(ass_loss, -1, name="ass_loss")  # [-1,1]
            ...
            ave_loss_loss = tf.reduce_sum(loss_loss)
